...
class Grille {
  ...
  chute() {
    for (let c = this.nbLignes - 1; c >= 0; c--) {
      for (let l = this.nbColonnes - 1; l > 0; l--) {
        if (this.tabCookies[l][c].htmlImage.classList.contains("cookies-cachee") && this.tabCookies[l - 1][c].marque === 0) {
          this.tabCookies[l][c].deselectionnee();
          let tmpType = this.tabCookies[l][c].type;
          let tmpImg = this.tabCookies[l][c].htmlImage.src;
          let tmp = this.tabCookies[l][c];

          this.tabCookies[l][c].type = this.tabCookies[l-1][c].type;
          this.tabCookies[l][c] = this.tabCookies[l-1][c];
          this.tabCookies[l][c].htmlImage.src = this.tabCookies[l-1][c].htmlImage.src;
          this.tabCookies[l][c].htmlImage.classList.remove("cookies-cachee");
          this.tabCookies[l-1][c].type = tmpType;
          this.tabCookies[l-1][c].marque = tmp;
          this.tabCookies[l-1][c].htmlImage.src = tmpImg;
          this.tabCookies[l-1][c].htmlImage.classList.add("cookies-cachee");
 ...
 aleatoire() {
    for (let l = 0; l < this.nbColonnes; l++) {
      for (let c = 0; c < this.nbLignes; c++) {
        if (this.tabCookies[l][c].marque === 1) {
          let img = Math.floor(Math.random()*6); // retourne un random entre 0 et 5
          let cookie1 = new Cookie(type, l, c);
          this.tabCookies[l][c].type = cookie1.img;
          this.tabCookies[l][c]= cookie1;
          this.tabCookies[l][c].htmlImage.src = cookie1.htmlImage.src;
          this.tabCookies[l][c].htmlImage.classList.remove("cookies-cachee");
          this.tabCookies[l][c].deselectionnee();
        }
      }
